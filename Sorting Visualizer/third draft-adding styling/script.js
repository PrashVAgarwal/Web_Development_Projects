...
function displayIteration(iterationIndex) {
  ...
  iterationBox.classList.add('iteration-box');
  
  const iterationNumbers=iterations[iterationIndex];
  const prevIterationNumbers = iterations[Math.max(0, iterationIndex - 1)];
  
  iterationNumbers.forEach((number,index)=>{
    const box=document.createElement('div');
    box.textContent=number;
    console.log(number);
    box.classList.add('number-box');
    iterationBox.appendChild(box);
    
    const prevNumber = prevIterationNumbers[index];
    if (prevNumber !== undefined && number !== prevNumber) {
      // Apply the swap animation class only to the elements that are being swapped
      box.classList.add('swap-animation');
      setTimeout(() => {
        box.classList.remove('swap-animation');
      }, 500); // Remove the class after the animation duration
  }});
  
  resultDiv.innerHTML = '';
  resultDiv.innerHTML = `Iteration ${iterationIndex + 1}:`;  
  resultDiv.appendChild(iterationBox);
  //${iterations[iterationIndex].join(', ')};
}
